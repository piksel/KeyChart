...
namespace KeyChart.Keyboards.QMK
{
    public class QmkClient
    {
        ...
        public static async Task<KeyboardInfo?> KeyboardInfo(string? selectedKeyboard)
        {
            ...
            using var stream = await wc.OpenReadTaskAsync("https://api.qmk.fm/v1/keyboards/" + selectedKeyboard);
        ...
        public static async Task<IReadOnlyCollection<string>?> Keyboards()
        {
            ...
            using var stream = await wc.OpenReadTaskAsync("https://api.qmk.fm/v1/keyboards");
        ...
        public static async Task<KeyMap?> DefaultKeymap(string keyboardId)
        {
            ...
            using var stream = await wc.OpenReadTaskAsync($"https://config.qmk.fm/keymaps/{keymapFile[0]}/{keymapFile}");
